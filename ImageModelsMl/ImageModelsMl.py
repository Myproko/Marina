...
#resnet_model = build_resnet50_ft().to(DEVICE)
#efficientnet_model = build_efficientnetb0_ft().to(DEVICE)

# ---------- training utils ----------
def train_model(model, train_loader, val_loader, epochs=EPOCHS, lr=1e-4):
...
# ---------- train models ----------
history_small = train_model(small_cnn, train_small, val_small)
#history_resnet = train_model(resnet_model, train_big, val_big, lr=1e-4)
#history_efficient = train_model(efficientnet_model, train_big, val_big, lr=1e-4)
...
print("Start evaluation of the Small CNN...")
test_res_small = evaluate_model(small_cnn, test_small)
print("✅ Small CNN is ready ")

print("\nTest (Small CNN)  -> Loss: {:.4f}  Acc: {:.4f}  AUC: {:.4f}".format(*test_res_small[:3]))
...
plot_history(history_small, "Small CNN")

# ---------- ROC curves ----------
plt.figure(figsize=(8,6))
plt.plot(test_res_small[3], test_res_small[4], label=f'Small CNN (AUC={test_res_small[2]:.3f})')
plt.plot([0,1],[0,1],'k--')
